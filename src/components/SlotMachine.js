...
export class SlotMachine {
  ...
  _createChrome() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      reelBg.fillRect(this.x + 20 + i * REEL_W, this.y + 20, REEL_W, REEL_H);
    }

    this.resultRow = this.scene.add.image(this.x + SLOT_W / 2, this.y + SLOT_H / 2, 'reel-result-row')
      .setOrigin(0.5)
      .setAlpha(0.8);
  }
  ...
  _createReels() {
    for (let i = 0; i < 5; i++) {
      const reelX = this.x + 20 + i * REEL_W + REEL_W / 2;
      const reelY = this.y + 20 + REEL_H / 2;
      ...
      const container = this.scene.add.container(reelX, reelY);
      container.setMask(mask);

      const flash = this.scene.add.graphics();
      flash.fillStyle(COLOR.WHITE, 1);
      flash.fillRect(this.x + 20 + i * REEL_W, this.y + SLOT_H / 2 - SYMBOL_H / 2, REEL_W, SYMBOL_H);
      flash.setAlpha(0);
  ...
  _makeSymbol(symbolDef, label) {
    ...
    if (symbolDef.id === 'number') {
      ...
      bg.fillRoundedRect(-REEL_W / 2 + 8, -SYMBOL_H / 2, REEL_W - 16, SYMBOL_H, 10);
      bg.strokeRoundedRect(-REEL_W / 2 + 8, -SYMBOL_H / 2, REEL_W - 16, SYMBOL_H, 10);
      const txt = this.scene.add.text(0, 0, String(label), {
        ...FONT.NUMBER, fontSize: '32px', color: '#F0F0FF',
      }).setOrigin(0.5);
      container.add([bg, txt]);
    } else if (symbolDef.id === 'jackpot') {
      container.add(this.scene.add.image(0, 0, 'sym-jackpot').setOrigin(0.5));
    } else if (symbolDef.id === 'wild') {
      container.add(this.scene.add.image(0, 0, 'sym-wild').setOrigin(0.5));
    } else if (symbolDef.id === 'multiplier') {
      container.add(this.scene.add.image(0, 0, 'sym-multiplier').setOrigin(0.5));
    }
    return container;
  }
  ...
  spin(results, fast = false) {
    ...
    this.reels.forEach((reel, i) => {
      ...
      // Start position: show symbols near the end of strip (above visible area)
      const startIndex = 0;
      const startOffset = (startIndex - targetIndex) * (SYMBOL_H + SYMBOL_GAP);
  ...
  destroy() {
    ...
    this.masks = [];
    if (this.resultRow) this.resultRow.destroy();
  }
}
